...
import fantasy.livematch.firstscore.activity.MoreAppActivity;
...
public class MoreAppDataAsync {
    private Activity activity;
    private JSONObject jObject;
    UrlController urlControl;

    public MoreAppDataAsync(final Activity activity) {
        ...
        try {
            ...
            Call<ResponseModel> call = apiService.GetMoreApps(jObject.toString());
    ...
    private void onPostExecute(ResponseModel responseModel) {
        try {
            ...
            if (responseModel.getStatus().equals(UrlController.STATUS_SUCCESS)) {
                ((MoreAppActivity) activity).setData(responseModel);
